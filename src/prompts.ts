...
【分析手順（ツールは3つ、出力順に合わせて実行）】
1) detect_forming_chart_patterns（形成中の大型パターン）
   - pair: btc_jpy
   - type: 1day
   - limit: 150
   ...
   - includeCompleted: false  // 形成中のみ、無効化済みは除外
   
   **重要**: includeCompleted=falseにより、形成中（forming/near_completion）のパターンのみが返される

3) detect_patterns（完成済みの大型パターン）
   - pair: btc_jpy
   - type: 1day
   - limit: 180
   - patterns: 同上
...
3) analyze_candle_patterns（短期2本足パターン）
   - pair: btc_jpy
   - type: 1day
