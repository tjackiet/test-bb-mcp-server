...
---
⚠️ 免責事項
※この分析は参考情報です。投資判断はご自身の責任で行ってください。相場は常に変動しており、将来の動きを保証するものではありません。` }]
      }
    ],
    metadata: {
      level: PromptLevel.BEGINNER,
...
※この分析は参考情報です。投資判断はご自身の責任で行ってください。相場は常に変動しており、将来の動きを保証するものではありません。`
  ...
  },
  {
    ...
    description: '中級者向け：完成済みパターンと形成中パターンを統合的に分析し、現在価格への影響を評価',
    messages: [
      {
        role: 'assistant',
        content: [{
...
【目的】
過去6ヶ月の完成済みパターンと現在形成中のパターンを網羅的に検出し、現在価格への影響を評価する

【最重要ルール】
**完成後80日以上経過したパターンは、回答のどこにも記載しない（存在しないものとして扱う）**

【出力の構成（時系列の自然な流れ）】
1. 形成中パターン（現在進行形、切迫性順）
2. 完成済みパターン（影響度順、**完成後79日以内のみ**）+ 過去パターンの位置づけ（低影響度を簡潔に、**完成後79日以内のみ**）
3. 重要パターンの解釈（データ再掲を廃止、解釈のみ記載）
   - 三尊が示唆すること（2行）
   - フォーリングウェッジが示唆すること（2行）
   - 2つのパターンの相互作用（2行）
   - 今後数日の注目点（箇条書き3項目）
4. 今後のシナリオ（強気📈/弱気📉/中立⏸️）
5. 現在の市場状況（1段落の要約）

※**絶対厳守**: 完成後80日以上経過したパターンは、全てのセクションで一切言及しない。
※**重要**: セクション3ではデータ（価格・日付）を再掲しない。解釈のみ記載。
...
- **鮮度フィルタリング（最重要）**: 完成後80日以上経過したパターンは**完全に無視**
  ...
  - 検出されても存在しないものとして扱う
  - 例: ダブルトップ（完成後88日） → 回答に一切登場させない

- **影響度判定（最優先ルール）**: 近接性チェックを必ず最初に実施
  - ネックライン/ターゲットから10%以上離れている → **自動的に低⚪**
  - 「中🟡」に記載されるのは、近接性がある（±10%以内）パターンのみ
  - 例: ダブルボトム（ネックラインから-24%離れている） → 低⚪「過去パターンの位置づけ」に記載

- **重複排除（厳格）**: 同じデータを複数箇所で記載しない
  - セクション1・2: データ（価格・日付・構造）を記載
  - セクション3: データ再掲禁止、解釈のみ記載
  - セクション4: シナリオのトリガー価格のみ記載（データ再掲禁止）

- 簡潔化: 構造的特徴の統計データ（傾き、タッチ回数、適合度r²、収束率など）は出力しない。
  - ウェッジ系: 上側/下側ラインの詳細は不要、ブレイクレベル（価格）のみ記載
  - 三尊/ダブル系: 主要な山/谷とネックラインのみ記載

【分析手順（ツールは2つ、出力順に合わせて実行）】
1) detect_forming_chart_patterns（形成中パターン）
   - pair: btc_jpy
   - type: 1day
   - limit: 150
   ...
   - view: detailed
   - maxBarsFromLastPivot: 10
   - includeCompleted: true
   - maxCompletedBars: 10

2) detect_patterns（完成済みパターン）
   - pair: btc_jpy
   - type: 1day
   - limit: 180
   - patterns: 同上
...
【出力フォーマット】
※時系列の自然な流れ: 形成中→完成済み→まとめ→シナリオ→要約
...
- **逆三尊**（M/D-M/D、完成後X日）: ネックラインXX,XXX,XXX円から±X%離れており、三尊に打ち消され済み → 影響度: 低
...
## 3. 重要パターンの解釈（2つのパターンが示唆すること）
※データ（価格・日付等）はセクション1・2に記載済みのため、ここでは解釈のみを記載

### 市場が示す2つのシグナルの拮抗

#### 三尊が示唆すること
（2行程度）XX月末から続いた下落トレンドは理論的な終点（XX,XXX,XXX円）に到達済み。整合度X.XXの典型的なパターンのため、この水準は強力なサポートとして機能する可能性。

#### フォーリングウェッジが示唆すること
（2行程度）本日アペックス到達により、方向性が決まる最終局面に突入。上方ブレイクなら強気転換、下方ブレイクなら三尊パターンを超えた下落継続を示唆。

#### 2つのパターンの相互作用
（2行程度）三尊が「ここまで下がった」を示し、ウェッジが「ここから反転か継続か」を問いかけている。三尊ターゲット（XX,XXX,XXX円）とウェッジ下限が近接しており、この水準が最重要サポートライン。

#### 今後数日の注目点
1. **XX,XXX,XXX円（三尊ターゲット）の維持**: ここを明確に割り込むと、三尊パターンの想定を超えた下落
2. **XX,XXX,XXX円（ウェッジ上限）の突破**: ここを超えると強気転換のシグナル
3. **ブレイクアウト時の出来高**: 急増すれば信頼性の高いブレイクアウト
...
BTC/JPYは現在、極めて重要な局面にあります。フォーリングウェッジがアペックス到達（本日MM/DD）で今日〜数日以内にブレイクアウトが予想され、三尊の理論ターゲット（XX,XXX,XXX円、到達度X%）に近接しており下落トレンドの終点に到達中です。上方ブレイク（XX,XXX,XXX円突破）で強気転換、下方ブレイク（XX,XXX,XXX円割れ）で弱気継続と、明確な分岐点を迎えています。

【統計情報】
- 検出パターン総数: X個（完成済みX個、形成中X個）
  ※完成後80日以上経過したパターンは除外済み（検出されても計上しない）
- 高影響度パターン: X個（極めて高い切迫性: X個含む）
- フィルタリング除外: X個（近接性0点で影響度極低）
  ※80日以上経過パターンは統計に含めない（言及不要）

【視覚化】
チャートは出さない（ユーザーが明示的に希望した場合のみ render_chart_svg を使用）
