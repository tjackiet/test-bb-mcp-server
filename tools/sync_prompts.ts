...
const RENDER_GUARD = 'チャート描画はユーザーが明示的に「見せて/表示/描画/出力/生成」の語で依頼した場合のみ実行してください。「チャートを使って分析」「チャートで確認」などは描画を意味しません（数値ツールで分析のみ）。';
...
const prompts = {
  bb_light_chart: {
    ...
    messages: [
      { role: 'system', content: [{ type: 'text', text: RENDER_GUARD }] },
      { role: 'assistant', content: [{ type: 'tool_code', tool_name: 'render_chart_svg', tool_input: { pair: '{{pair}}', type: '{{type}}', limit: '{{limit}}', withBB: true, bbMode: 'default', withSMA: [] } }] }
    ]
  },
  bb_full_chart: {
    ...
    messages: [
      { role: 'system', content: [{ type: 'text', text: RENDER_GUARD }] },
      { role: 'assistant', content: [{ type: 'tool_code', tool_name: 'render_chart_svg', tool_input: { pair: '{{pair}}', type: '{{type}}', limit: '{{limit}}', withBB: true, bbMode: 'extended', withSMA: [] } }] }
    ]
  },
  ichimoku_default_chart: {
    ...
    messages: [
      { role: 'system', content: [{ type: 'text', text: RENDER_GUARD }] },
      { role: 'assistant', content: [{ type: 'tool_code', tool_name: 'render_chart_svg', tool_input: { pair: '{{pair}}', type: '{{type}}', limit: '{{limit}}', withIchimoku: true, ichimoku: { mode: 'default' }, withSMA: [] } }] }
    ]
  },
  ichimoku_extended_chart: {
    ...
    messages: [
      { role: 'system', content: [{ type: 'text', text: RENDER_GUARD }] },
      { role: 'assistant', content: [{ type: 'tool_code', tool_name: 'render_chart_svg', tool_input: { pair: '{{pair}}', type: '{{type}}', limit: '{{limit}}', withIchimoku: true, ichimoku: { mode: 'extended' }, withSMA: [] } }] }
    ]
  },
  patterns_analysis: {
  ...
  candles_only_chart: {
    ...
    messages: [
      { role: 'system', content: [{ type: 'text', text: RENDER_GUARD }] },
      { role: 'system', content: [{ type: 'text', text: '以下は明示要求があった場合の最小設定。分析目的だけでは render_chart_svg を呼ばないでください。' }] },
