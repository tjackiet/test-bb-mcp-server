...
export default async function analyzeMarketSignal(
  ...
  try {
    const [flowRes, volRes, indRes] = await Promise.all([
      ...
      getIndicators(chk.pair, type, 120) as any,
    ]);
    ...
    // Composite score
    const weights = { buyPressure: 0.4, cvdTrend: 0.3, momentum: 0.2, volatility: 0.1 } as const;
    const score = Number((
      buyPressure * weights.buyPressure +
      cvdTrend * weights.cvdTrend +
    ...
    const summary = formatSummary({
      ...
      extra: `score=${score} rec=${recommendation}`,
    });
