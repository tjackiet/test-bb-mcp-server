...
async function generateReport() {
  ...
  targetDate.setDate(targetDate.getDate() - 1); // 昨日
  ...
  try {
    ...
    for (const file of logFiles) {
      ...
      for await (const line of rl) {
        try {
          ...
          if (log.ts && log.ts.startsWith(dateStr) && log.type === 'tool_run') {
            stats.total++;
            if (log.result?.ok) {
              stats.success++;
            } else {
    ...
    const errorTypesSummary = Object.entries(stats.errorTypes)
...
`;
    ...
    fs.writeFileSync(reportFile, reportContent.trim(), 'utf8');
